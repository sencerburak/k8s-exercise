...
def restart_deployments(api, apps_api):
    ...
    deps = apps_api.list_namespaced_deployment(namespace=NS, label_selector=LABEL_SELECTOR)
    
    if not deps.items:
        logging.warning("No deployments found with selector %s in namespace %s", LABEL_SELECTOR, NS)
        return
...
def main():
    ...
    logging.info("Starting watcher for ConfigMap '%s' in namespace '%s' with selector '%s'",
                 CM_NAME, NS, LABEL_SELECTOR)
    
    while True:
        try:
            stream = w.stream(core.list_namespaced_config_map,
                              namespace=NS,
                              timeout_seconds=0)
            for ev in stream:
                etype = ev["type"]
